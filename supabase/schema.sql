...
create table if not exists public.br_rounds (
  ...
  time_limit_seconds integer not null default 120 check (time_limit_seconds between 5 and 300),
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_rounds) loop
    ...
    v_time := greatest(5, least(coalesce(nullif(v_item->>'time_limit_seconds', '')::integer, 120), 300));
...
create or replace function public.br_auto_advance_game(p_invite_code text)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if v_game.status <> 'running' or v_game.current_round_index < 0 then
    return public.br_public_game_state(p_invite_code);
  end if;
  ...
  if v_game.round_started_at is null or now() < v_game.round_started_at + make_interval(secs => v_round.time_limit_seconds) then
    return public.br_public_game_state(p_invite_code);
  end if;

  perform public.br_process_current_timeouts(v_game.id);

  select count(*) into v_alive
  from public.br_players
  where game_id = v_game.id and eliminated = false and lives > 0;
  ...
  if v_alive <= 1 or v_game.current_round_index + 1 >= v_round_count then
    update public.br_games
    set status = 'finished', updated_at = now()
    where id = v_game.id and status = 'running';
  else
    v_next := v_game.current_round_index + 1;
    update public.br_games
    set current_round_index = v_next,
        round_started_at = now(),
        updated_at = now()
    where id = v_game.id and status = 'running' and current_round_index = v_game.current_round_index;
  end if;
...
grant execute on function public.br_auto_advance_game(text) to anon, authenticated;
