...
const DEFAULT_ROUNDS = [
  ...
  },
  {
    ...
    question_text: 'Продолжите ряд: 2, 4, 8, 16, ...',
    option_a: '18',
    option_b: '24',
    option_c: '32',
    option_d: '64',
  ...
  },
  {
    ...
    type: 'trap',
    title: 'Ловушка',
    question_text: 'Не торопись: у отца Марины 5 дочерей: Лала, Леле, Лили, Лоло. Как зовут пятую?',
    option_a: 'Лулу',
    option_b: 'Марина',
    option_c: 'Лала',
    option_d: 'Леля',
  ...
  },
  {
    ...
    type: 'immunity',
    title: 'Щит',
    question_text: 'Что можно держать после того, как отдал другому?',
    option_a: 'Слово',
    option_b: 'Телефон',
    option_c: 'Ключ',
    option_d: 'Деньги',
  ...
  },
  {
    ...
    type: 'reflex',
    title: 'Реакция',
    question_text: 'Мини-игра: успей нажать красную цель до конца таймера.',
  ...
  },
  {
    ...
    type: 'memory',
    title: 'Память',
    question_text: 'Запомни код ведущего: BR-7-2-9. Какой код был показан?',
    option_a: 'BR-7-2-9',
    option_b: 'BR-2-7-9',
    option_c: 'BR-7-9-2',
    option_d: 'BR-9-7-2',
  ...
  },
  {
    ...
    question_text: 'Финальный риск: какое слово всегда написано неправильно?',
    option_a: 'Ошибка',
    option_b: 'Неправильно',
    option_c: 'Всегда',
    option_d: 'Слово',
    correct_option: 'B',
    time_limit_seconds: 10,
...
function renderShell(content, mode = 'page') {
  setMode(mode === 'host' ? 'host' : mode === 'game' ? 'game' : 'page');
    ...
    <div class="page">
      <section class="hero">
        <div class="hero-inner">
          ...
          <p class="hero-lead">Вопросы, мини-игры, спецраунды, 3 жизни и только один победитель.</p>
          <div class="hero-date">
          ...
          <div class="hero-stats">
            ...
            <div class="stat-card"><b>x2</b><span>спецраунды</span></div>
      ...
      <section class="host-layout">
        <aside class="host-side glass">
          ...
          <div class="form-grid">
            ...
            <button class="primary-btn" id="create-game">Создать с готовыми раундами</button>
          </div>
        ...
        <main class="host-main glass">
          ...
          <details>
            ...
            <p class="tiny">Можно менять вопросы, варианты, типы и таймер. Типы: quiz, fast, double, immunity, trap, sudden, reflex, memory.</p>
...
async function renderHostGame(gameId) {
  ...
  function renderHostState(state) {
    ...
    $('#host-game-content').innerHTML = `
      ...
      <div class="copy-box">
        <input readonly value="${escapeHtml(inviteUrl(game.invite_code))}" />
        <button class="primary-btn" id="copy-invite">Скопировать ссылку</button>
      </div>
      <div class="kpi-grid">
    ...
    $('#copy-invite').onclick = () => copyText(inviteUrl(game.invite_code));
    $('#start-game').onclick = () => hostAction('br_host_start_game', 'Игра запущена');
    $('#next-round').onclick = () => hostAction('br_host_next_round', 'Следующий раунд запущен');
    $('#finish-game').onclick = () => hostAction('br_host_finish_game', 'Игра завершена');
    ...
    <div class="game-shell">
      <section class="lobby-hero">
        <div class="lobby-content">
          <div class="eyebrow">Код игры: ${escapeHtml(state.game.invite_code)}</div>
          <h1 class="big-title">Лобби <span>игры</span></h1>
          <p class="hero-lead">Ожидание игроков. Ведущий скоро запустит первый раунд.</p>
          <div class="hero-stats">
            <div class="stat-card"><b>${(state.players || []).length}</b><span>игроков в лобби</span></div>
      ...
      <section class="game-grid">
        <div class="status-panel glass">
          ...
          <div class="rule-list">
            ${renderRule(1, 'Отвечай быстро', 'На каждый вопрос или мини-игру есть таймер.')}
            ${renderRule(2, 'Береги жизни', 'Ошибка или тайм-аут снимает жизнь.')}
            ${renderRule(3, 'Лови бонусы', 'Иммунитет спасает от ошибки, x2 даёт больше очков.')}
          </div>
        </div>
