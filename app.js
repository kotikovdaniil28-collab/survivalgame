...
let autoAdvanceKey = '';
let countdownZeroFiredFor = '';
let autoAdvanceInFlight = false;

const PLAYER_ANSWER_STORAGE_PREFIX = 'br_survival_answer_';
function answerStorageKey(code, roundId) {
  return `${PLAYER_ANSWER_STORAGE_PREFIX}${normalizeCode(code)}_${roundId}`;
}
function setStoredRoundAnswer(code, roundId, answer, isCorrect, usedShield) {
  try {
    localStorage.setItem(answerStorageKey(code, roundId), JSON.stringify({ answer, isCorrect: !!isCorrect, usedShield: !!usedShield, at: Date.now() }));
  } catch {}
}
function getStoredRoundAnswer(code, roundId) {
  try {
    const raw = localStorage.getItem(answerStorageKey(code, roundId));
    return raw ? JSON.parse(raw) : null;
  } catch { return null; }
}
function optionStateClass(key, hasAnswered, stored) {
  if (!hasAnswered) return '';
  if (!stored || !stored.answer) return 'locked';
  if (stored.answer !== key) return 'locked';
  if (stored.usedShield) return 'selected-shield';
  return stored.isCorrect ? 'selected-correct' : 'selected-wrong';
}
...
const audio = (() => {
  ...
  function preload() {
    Object.entries(files).forEach(([name, src]) => {
      ...
      a.preload = 'auto';
      a.volume = name === 'ambient' ? 0.05 : 0.32;
  ...
  function background() {
    ...
    if (bgAudio && bgAudio.paused) {
      try { bgAudio.currentTime = bgAudio.currentTime || 0; bgAudio.volume = 0.045; bgAudio.play().catch(() => {}); } catch {}
    }
    // фон без постоянного сердцебиения: меньше раздражающих повторов
  }
  ...
  return {
    ...
    click() { playFile('tick', .18); tone(280, .045, 'triangle', .018, 360); },
    start() { playFile('start', .34); tone(110, .28, 'sine', .035, 164); tone(330, .10, 'triangle', .022, 494, .12); },
    round() { playFile('round', .30); tone(120, .20, 'triangle', .032, 180); tone(360, .09, 'sine', .020, 540, .09); },
    correct() { playFile('correct', .32); tone(520, .09, 'sine', .032, 760); tone(760, .10, 'triangle', .024, 980, .08); },
    wrong() { playFile('wrong', .30); tone(180, .18, 'triangle', .035, 110); },
    death() { playFile('death', .34); tone(120, .42, 'sine', .040, 70); },
    timeout() { playFile('timeout', .26); tone(240, .09, 'triangle', .025, 150); },
    victory() { playFile('victory', .36); [392, 523, 659, 784].forEach((f, i) => tone(f, .16, 'triangle', .035, null, i * .11)); },
    tick(second) { if (muted || second === lastTickSecond) return; lastTickSecond = second; if (second <= 5 && second > 0) { playFile('tick', .20); tone(640, .032, 'triangle', .020, 480); } },
    boom: scaryHit,
    background,
    stopBackground,
    resetTick
...
function normalizeRoundForGame(round, index) {
  const copy = { ...round };
  const keys = ['A', 'B', 'C', 'D'];
  const oldCorrect = String(copy.correct_option || '').toUpperCase();
  const oldOptions = keys.map(key => ({ key, text: copy[`option_${key.toLowerCase()}`] || key }));
  const correctText = (oldOptions.find(o => o.key === oldCorrect) || oldOptions[0]).text;
  const shuffledOptions = shuffleArray(oldOptions);
  shuffledOptions.forEach((item, optionIndex) => {
    const newKey = keys[optionIndex];
    copy[`option_${newKey.toLowerCase()}`] = item.text;
    if (item.text === correctText && item.key === oldCorrect) copy.correct_option = newKey;
  });
  if (!keys.includes(String(copy.correct_option || '').toUpperCase())) {
    const idx = shuffledOptions.findIndex(item => item.key === oldCorrect);
    copy.correct_option = keys[Math.max(0, idx)];
  }
  copy.round_order = index;
  copy.time_limit_seconds = copy.type === 'reflex' ? 30 : 120;
  return copy;
}
function prepareRoundsForGame(rounds) {
  return shuffleArray(Array.isArray(rounds) ? rounds : DEFAULT_ROUNDS)
    .map((round, index) => normalizeRoundForGame(round, index));
}
...
async function requestAutoAdvance(roundId, endIso) {
  if (!currentGameCode || !roundId) return;
  const key = `${normalizeCode(currentGameCode)}:${roundId}:${endIso || ''}`;
  if (autoAdvanceInFlight || autoAdvanceKey === key) return;
  autoAdvanceKey = key;
  autoAdvanceInFlight = true;
  try {
    await new Promise(resolve => setTimeout(resolve, 900));
    await rpc('br_auto_advance_game', { p_invite_code: currentGameCode });
    if (document.body.classList.contains('broadcast-mode')) await refreshBroadcastGame();
    else if (document.body.classList.contains('game-mode')) await renderPublicGame();
  } catch (e) {
    // Если SQL ещё не обновили, просто молча оставляем ручное управление.
    console.warn('auto advance skipped:', e?.message || e);
  } finally {
...
async function renderHostList() {
      ...
      <section class="host-layout">
        ...
        <main class="host-main glass">
          ...
          <details>
            ...
            <p class="tiny">Встроено 30 сложных раундов. При создании игры порядок вопросов и варианты ответов перемешиваются автоматически. Обычные вопросы идут по 2 минуты, мини-игра реакции — 30 секунд.</p>
  ...
  async function createGame() {
    try {
      const rounds = prepareRoundsForGame(JSON.parse($('#rounds-json').value));
...
function renderRunning(state, me) {
  ...
  const storedAnswer = round ? getStoredRoundAnswer(currentGameCode, round.id) : null;
      ...
      <section class="game-grid">
        <main class="stage glass">
          <div class="round-top">
            ...
            <div class="timer" data-end="${escapeHtml(end)}" data-round-id="${round ? escapeHtml(round.id) : ''}">--</div>
          </div>
          ${eliminated ? renderEliminated(me) : renderRoundBody(round, hasAnswered, storedAnswer)}
        </main>
        <aside class="leaderboard glass">
          <div class="row-between">
            <h2>Таблица</h2>
...
function renderRoundBody(round, hasAnswered, storedAnswer = null) {
  ...
  return `
    <p class="question-desc">${escapeHtml(round.question_text)}</p>
    <div class="options">
      ${['A','B','C','D'].map(key => `<button class="option-btn ${optionStateClass(key, hasAnswered, storedAnswer)}" data-answer="${key}" ${hasAnswered ? 'disabled' : ''}><b>${key}</b>${escapeHtml(round[`option_${key.toLowerCase()}`])}</button>`).join('')}
    </div>
    ${already}
  `;
}
...
async function submitAnswer(roundId, answer) {
  try {
    ...
    setStoredRoundAnswer(currentGameCode, roundId, answer, result.is_correct, result.used_shield);
    if (result.is_correct) {
      ...
      audio.wrong();
      toast(`Неверно: ${Math.abs(result.lives_delta)} жизнь снята`, 'bad');
    }
    await renderPublicGame();
  } catch (e) {
...
function startCountdown() {
  ...
  if (!timer) return;
  const endIso = timer.dataset.end || '';
  const roundId = timer.dataset.roundId || '';
  ...
  const tick = () => {
    ...
    audio.tick(left);
    if (left === 0) {
      if (countdownZeroFiredFor !== `${roundId}:${endIso}`) {
        countdownZeroFiredFor = `${roundId}:${endIso}`;
        audio.timeout();
        requestAutoAdvance(roundId, endIso);
      }
      stopCountdown();
    }
    timer.style.borderColor = left <= 10 ? 'rgba(255,81,81,.82)' : 'rgba(255,45,37,.58)';
  };
  tick();
    ...
    <div class="broadcast-shell broadcast-game-bg">
      ...
      <main class="broadcast-stage glass ${round?.type || ''}">
        <div class="broadcast-round-head">
          ...
          <div class="timer broadcast-timer" data-end="${escapeHtml(end)}" data-round-id="${round ? escapeHtml(round.id) : ''}">--</div>
        </div>
        ${round ? `
