...
const audio = (() => {
  ...
  function gain(value, time = 0.2) {
    ...
    g.gain.exponentialRampToValueAtTime(Math.max(0.0001, value), c.currentTime + 0.015);
    g.gain.exponentialRampToValueAtTime(0.0001, c.currentTime + time);
    g.connect(c.destination);
    return g;
  }
  function tone(freq = 440, duration = .2, type = 'sine', volume = .16, slideTo = null) {
    const c = init();
    if (!c) return;
    const osc = c.createOscillator();
    const g = gain(volume, duration);
    if (!g) return;
    osc.type = type;
    ...
    osc.connect(g);
    osc.start();
    osc.stop(c.currentTime + duration + .03);
  }
  ...
  function background() {
    ...
    pulse.type = 'sine';
    pulse.frequency.setValueAtTime(1.1, c.currentTime);
    pulseGain.gain.setValueAtTime(0.045, c.currentTime);
    mainGain.gain.setValueAtTime(0.035, c.currentTime);
    pulse.connect(pulseGain);
    pulseGain.connect(mainGain.gain);
    osc.connect(mainGain);
    mainGain.connect(c.destination);
    osc.start(); pulse.start();
    bg = [osc, pulse, mainGain, pulseGain];
  }
  function stopBackground() {
    bg.forEach(node => { try { if (node.stop) node.stop(); if (node.disconnect) node.disconnect(); } catch {} });
    bg = [];
  }
  return {
    isMuted: () => muted,
    init,
    toggle() { muted = !muted; localStorage.setItem('br_survival_muted', muted ? '1' : '0'); if (muted) stopBackground(); return muted; },
    click() { tone(220, .07, 'square', .055, 330); },
    start() { tone(180, .12, 'sawtooth', .10, 360); setTimeout(() => tone(520, .18, 'triangle', .12, 780), 90); noise(.08, .035); },
    round() { tone(110, .16, 'sawtooth', .08, 220); setTimeout(() => tone(440, .16, 'triangle', .08, 660), 120); },
    correct() { tone(440, .12, 'sine', .13, 880); setTimeout(() => tone(660, .12, 'triangle', .10, 990), 110); },
    wrong() { tone(155, .26, 'sawtooth', .15, 55); noise(.18, .06); },
    death() { stopBackground(); tone(100, .9, 'square', .18, 25); setTimeout(() => noise(.55, .08), 120); },
    timeout() { tone(260, .10, 'square', .10, 120); setTimeout(() => tone(180, .12, 'square', .08, 90), 120); },
    victory() { stopBackground(); [392, 523, 659, 784].forEach((f, i) => setTimeout(() => tone(f, .18, 'triangle', .11), i * 120)); },
    background,
    stopBackground
  };
})();
...
function setupSoundToggle() {
  ...
  if (!btn) return;
  const sync = () => { btn.textContent = audio.isMuted() ? '🔇' : '🔊'; btn.setAttribute('aria-label', audio.isMuted() ? 'Звук выключен' : 'Звук включён'); };
  btn.onclick = () => { audio.init(); audio.toggle(); sync(); if (!audio.isMuted()) audio.click(); };
  sync();
}
